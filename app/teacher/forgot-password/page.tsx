      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.3 }}
        ...
        {sent ? (
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
            ...
            <Link href="/teacher/login" className="block text-sm text-[#1B2B4B] font-medium hover:text-[#F59E0B]">Back to Sign In</Link>
          </motion.div>
        ) : (
          <form onSubmit={handleSubmit} className="border border-gray-200 rounded-xl p-8 space-y-5">
            ...
            <p className="text-center text-sm text-gray-500">
              <Link href="/teacher/login" className="text-[#1B2B4B] hover:text-[#F59E0B]">Back to Sign In</Link>
            </p>
          </form>
        )}
      </motion.div>
